        ...
        <template slot-scope="{ row, rowIndex }">
          <div
            v-html="
              ((value, row, rowIndex) => {
                if (row.intflag == 0) {
                  return `<span style='font-size: 12px'>未审核</span>`;
                } else if(row.intflag == 1&&row.intcashflag==1){
                   return `<span style='font-size: 12px'>未出纳</span>`;
                }else if(row.intflag==1&&row.intcashflag==2){
                    return `<span style='font-size: 12px'>已出纳</span>`;
                }else if(row.intflag==1&&(row.intcashflag==0||row.intcashflag==2)){
                    return `<span style='font-size: 12px'>未记账</span>`;
                }else if(row.intflag==2){
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if (validatenull(this.currentRow)){
        this.showMessage({
          type: 'success',
          msg: '请选择关联的发票'
        })
      ...
      this.$emit("onGetData",this.currentRow.uqvoucherid);
    ...
    getList() {
      ...
      }else if(this.formData.intflag==1){
      ...
      else if(this.formData.intflag==2){
        this.search.params.push({
          columnName: "intflag",
          logic: "AND",
          queryType: "=",
      ...
      else if(this.formData.intflag==3){
        this.search.params.push({
          columnName: "intflag",
          logic: "AND",
          queryType: "=",
      ...
      else if(this.formData.intflag==4){
        this.search.params.push({
          columnName: "intflag",
          logic: "AND",
          queryType: "=",
      ...
      this.search.params.push({
        columnName: "uqglobalperiodid.INTYEARMONTH",
        logic: "AND",
        queryType: ">=",
