...
import { listAgingPage, deleteAging } from '@/api/tgl/aging/aging'
  ...
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 账龄区间tgl_aging
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listAgingRespData, listPermissionRespData] = await Promise.all([
          listAgingPage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listAgingRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listAgingRespData.data.total
          this.tableData = listAgingRespData.data.rows ? listAgingRespData.data.rows : []
          ...
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listAgingRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
