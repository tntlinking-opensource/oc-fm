...
import { getTableName } from '@/api/tgl/ledger/accountBook.js'
    ...
    getQueryData(editForm) {
      ...
      getTableName(this.queryFormData).then((responseData) => {
        if (responseData.code == 100) {
          ...
          const name = 'mysql-provider-accountBook.ureport.xml' // 报表名，固定
          this.src = this.baseURL + '/ureport/preview?_u=' + name + '&_i=1&_r=1&tableName=' + tableName + '&beginyearmonth=' + beginyearmonth + '&endyearmonth=' + endyearmonth + '&date=' + date + '&companyName=' + companyName + '&isseveral=' + isseveral
