  ...
  <el-row class="dc-container" v-loading="loading">
    ...
    >
      ...
      >
        ...
        {{mainformatDate(scope.row.chargeagainstsubmitodd)}}
        </template>
      </ux-table-column>
      <ux-table-column
            ...
            v-html="
              ((value, row, rowIndex) => {
                if (row.intflag == 0) {
                  ...
                  return `<span style='font-size: 12px'>已审核</span>`;
                }
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (
          ...
          this.permission.view = listPermissionRespData.data.find(item => {
            return item.permission === "tglVoucherMains:read";
          });
          this.permission.export = listPermissionRespData.data.find(item => {
            return item.permission === "tglVoucherMains:export";
          });

          this.permission.add = listPermissionRespData.data.find(item => {
            return item.permission === "tglVoucherMains:create";
          });
          this.permission.edit = listPermissionRespData.data.find(item => {
            return item.permission === "tglVoucherMains:update";
          });
          this.permission.remove = listPermissionRespData.data.find(item => {
            return item.permission === "tglVoucherMains:delete";
          });
        } else {
          this.showMessage(
            listPermissionRespData.code != 100
    ...
    onEdit(scope) {
      ...
      setTimeout(_ => {
        getTglVoucherMainsById(scope.row.uqvoucherid).then(res => {
          this.$refs.addCertificateDialog.onView(res.data,'edit');
        });
      }, 100);
    },
    onCopy(scope) {
      ...
      setTimeout(_ => {
        getTglVoucherMainsById(scope.row.uqvoucherid).then(res => {
          this.$refs.addCertificateDialog.onView(res.data,'edit',true);
        });
      }, 100);
    },
    onDelete(scope) {
      this.$confirm("确定删除吗？", "确认", {
        ...
        .then(() => {
          this.setLoad();
          var entitys=[];
          entitys.push(scope.row.uqvoucherid)
          deleteTglVoucherMains(entitys)
            .then(responseData => {
              if (responseData.code == 100) {
                this.getList();
                this.showMessage({
