      ...
      try {
        ...
        if (
          ...
          this.permission.view = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercheck:read";
          });
          this.permission.export = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercheck:export";
          });

          this.permission.add = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercheck:create";
          });
          this.permission.edit = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercheck:update";
          });
          this.permission.remove = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercheck:delete";
          });
        } else {
          this.showMessage(
            listPermissionRespData.code != 100
      ...
      try {
        checkBoxData.forEach(item => {
          if (item.intflag != 0 && code == "start") {
            throw new Error(`${item.intvouchernum}不是未审核，不能审核!`);
          } else if (item.intflag != 1 && code == "stop") {
            throw new Error(`${item.intvouchernum}不是已审核状态!`);
          } else {
        ...
        this.title = ` 选择${code == "start" ? "审核" : "反审核"}日期`;
      } catch (error) {
        this.showMessage({
          type: "warning",
          msg: error
