  ...
  <el-row class="dc-container" v-loading="loading">
      ...
      >
        <template slot-scope="{ row, rowIndex }">
          <div
            v-html="
              ((value, row, rowIndex) => {
                if (row.intcashflag == 1) {
                  return `<span style='font-size: 12px'>未出纳</span>`;
                } else if (row.intcashflag == 2) {
                  return `<span style='font-size: 12px'>已出纳</span>`;
                }
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitVoucherCash">确 定</el-button>
      </span>
    </el-dialog>
  </el-row>
</template>
      ...
      try {
        ...
        if (
          ...
          this.permission.view = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercash:read";
          });
          this.permission.export = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercash:export";
          });

          this.permission.add = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercash:create";
          });
          this.permission.edit = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercash:update";
          });
          this.permission.remove = listPermissionRespData.data.find(item => {
            return item.permission === "vouchercash:delete";
          });
        } else {
          this.showMessage(
            listPermissionRespData.code != 100
      ...
      try {
        checkBoxData.forEach(item => {
          ...
          if (item.intflag != 1 && code == "start") {
            throw new Error(`${item.intvouchernum}不是未出纳，不能出纳!`);
          } else if (item.intflag != 1 && code == "stop") {
            throw new Error(`${item.intvouchernum}不是已出纳状态!`);
          } else {
        ...
        this.title = ` 选择${code == "start" ? "出纳" : "反出纳"}日期`;
      } catch (error) {
        this.showMessage({
          type: "warning",
          msg: error
