<template>
  <div class="edit-container" :class="editPersonStyle? '' : 'no-bg'">
    <!-- 添加任务栏 -->
...
<script>
  ...
  const defaultLayout = Array.from(initPanelsLayout);
      ...
      listUserGridAll(this.search).then(result =>{             //查询用户自定义布局
        if(result.code === "100"){
          if(result.data.length === 0){   // 当前用户未自定义  把拥有的模块全部展示
            let Check = [];
            //选中的模块
            newPermission.forEach(attr => {
              this.panels.push(attr);            //  多选模块信息
              Check.push(attr.id);
            })
            ...
            this.initPanel();
              ...
              newPermission.forEach(attr => {
                this.panels.push(attr);          //  拥有权限的多选模块信息
              })
              this.initPanel();
            }
      ...
      initPanel () {
        ...
        if(this.layout.length > 0){
            ...
            this.initLayout.forEach((item, index) => {
              let items = defaultLayout.filter((x) => x.i == item.i)
              item.x = items[0].x;
              item.y = items[0].y;
              item.w = items[0].w;
              item.h = items[0].h;
