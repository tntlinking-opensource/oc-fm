        ...
        >
          <el-row>
            <el-form-item
              ...
              <el-input
                v-model="form.ledgetypeName"
                placeholder="请选择分户"
                style="width: 80%"
                disabled
                class="dc-el-input_SingleInput">
                slot="suffix"重点  btnClearable事件，value有值就显示没有值就隐藏
                <el-button type="info" plain v-on:click="onSearchLedgetype" icon="el-icon-search" slot="append"></el-button>
                <el-button type="danger" plain v-on:click="btnClearable" icon="el-icon-delete" slot="append"></el-button>
              </el-input>
            ...
            <el-form-item  label="科目从：" class="dc-el-form-item_NumberInput"
              ...
              <temlapte slot-scope="scope">
                <el-select v-model="form.startHkont" slot="suffix" :fetch-suggestions="queryHkont" style="width: 80%">
...
<!--                ></el-button>-->
            ...
            <el-form-item  label="到：" class="dc-el-form-item_NumberInput"
                           :rules="[{ required: true, message: '请选择科目', trigger: 'change' }]">
              <temlapte slot-scope="scope">
                <el-select v-model="form.endHkont" slot="suffix" :fetch-suggestions="queryHkont" style="width: 80%">
                  <el-option v-for="(item, index) in uqaccountidOptions"
                             :key="index"
                             :label="item.varaccountcode
                                  ? '[' + item.varaccountcode + ']' + item.varaccountname : item.varaccountname"
...
<!--                ></el-button>-->
  ...
  ></addHkontDialog2>
    <LedgetypeQuery ref="typeQuery" :appendToBody="true" checkType="radio"
                    :uqaccountid="this.form.uqaccountid"
...
import { getBookStatus,getHkontLevel,queryHkont } from '@/api/ureport/accountingBook/accountingBook'
    ...
    getTreeCheckData(data) {
      ...
      if (!validatenull(data)) {
        if (data.hasOwnProperty('ledgetypeid')) {
          this.form.ledgertypeid = data.ledgertypeid
          this.form.ledgerdeid = ''
        } else {
          this.form.ledgertypeid = ''
          this.form.ledgerdeid = data.ledgerid
        }
        this.form.ledgetypeName = data.ledgetypename
      }
    },
