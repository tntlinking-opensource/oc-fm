...
import conditionDialog from '@/views/ureport/ledgerglbook/conditionDialog'
...
export default {
  ...
  methods: {
    ...
    getQueryPeriods(data) {
      ...
      var ledgetypeCode = data.ledgetypeName.substring(1,data.ledgetypeName.indexOf("]")) ;
      var ledgetypeName = data.ledgetypeName.substring(data.ledgetypeName.indexOf("]")+1,data.ledgetypeName.length) ;
      ...
      this.src = this.baseURL + '/ureport/preview?_u=' + 'mysql-provider-ledgerGlBook.ureport.xml'
        + '&startPeriod=' + data.startPeriod + '&endPeriod=' + data.endPeriod + '&beginAccount=' + startHkont + '&endAccount=' + endHkont
        + '&intstatus=' + data.bookStatus +'&hkontLevel=' + data.hkontLevel +'&now='+ now
        + '&ledgerdeid='+data.ledgerdeid+'&ledgertypeid'+data.ledgertypeid
        + '&uqcompanyid=' + currentUser.company.id
        + '&ledgetypename=' + ledgetypeName
        + '&ledgetypecode=' + ledgetypeCode;
