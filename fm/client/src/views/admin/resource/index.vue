...
export default {
  ...
  methods: {
    onFastCreateResource() {
      ...
      this.resourceDefs.forEach(item => {
        let resource = {
          name: item.name + this.parentTable.name, // 名称
          code: this.parentTable.code, // 代码
          url: this.parentTable.url, // Url
          permission:
            this.parentTable.code.substr(this.parentTable.code.lastIndexOf('/') + 1) + ':' + item.permission, // 权限
          canPermission: '1', // 可授权
          isDefault: 0, // 是否默认资源
          isLocked: 0, // 禁用
          router: {
